...
int res = 2;//定义数字接口10
int dc = 0;//定义数字接口11
int cs = 30;//定义数字接口12  IO 口不够用，随便指定一个，cs 引脚硬件已接地
int MOTO_PIN = 4;// 电机控制端口
...
void setup() {
    ...
    LCD_Clear(BLACK);
    BACK_COLOR = BLACK;
    LCD_ShowString(28, 20, "Plant Care", RED);
    LCD_ShowString(100, 80, "Version 1.0", RED);
    delay(4000);
...
void loop() {
    ...
    LCD_Clear(BLACK);
    LCD_ShowString(4, 4, "", RED);
    LCD_ShowString(24, 4, "Status:", RED);
    LCD_ShowString(44, 4, "Cont:", RED);
    LCD_ShowString(64, 4, "Time:", RED);
    delay(2000);
